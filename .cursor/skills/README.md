...
2. **Cursor 识别**: `.cursor/skills/` 目录会被 Cursor 自动识别
3. **版本控制友好**: 只需提交 `skills/` 目录的源文件
4. **便于开发**: 修改源文件，Cursor 立即生效
...
skills/openclaw-manager/SKILL.md  (源文件)
    ...
    │ (软链接)
    ↓
.cursor/skills/openclaw-manager/SKILL.md
    ↓
Cursor AI 读取并使用
```
...
```bash
rm .cursor/skills/openclaw-manager
```
...
Cursor 会自动识别 `.cursor/skills/` 目录下的所有技能包。
