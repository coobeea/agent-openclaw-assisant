...
2. **Claude 识别**: `.claude/skills/` 目录会被 Claude Desktop 自动识别
3. **版本控制友好**: 只需提交 `skills/` 目录的源文件
4. **便于开发**: 修改源文件，Claude 立即生效
...
skills/openclaw-manager/SKILL.md  (源文件)
    ...
    │ (软链接)
    ↓
.claude/skills/openclaw-manager/SKILL.md
    ↓
Claude AI 读取并使用
```
...
```bash
rm .claude/skills/openclaw-manager
```
...
Claude Desktop 会自动识别 `.claude/skills/` 目录下的所有技能包。

在 Claude 中，直接用自然语言交互：
