...
2. **Trae 识别**: `.trae/skills/` 目录会被 Trae 自动识别
3. **版本控制友好**: 只需提交 `skills/` 目录的源文件
4. **便于开发**: 修改源文件，Trae 立即生效
...
skills/openclaw-manager/SKILL.md  (源文件)
    ...
    │ (软链接)
    ↓
.trae/skills/openclaw-manager/SKILL.md
    ↓
Trae AI 读取并使用
```
...
```bash
rm .trae/skills/openclaw-manager
```
...
Trae 会自动识别 `.trae/skills/` 目录下的所有技能包。
